<template>
  <div class="space-y-6">
    ...
    <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-soft flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h2 class="text-lg font-bold text-slate-900">Live Stock Balances</h2>
        <p class="text-xs text-slate-500 mt-0.5">Real-time bin quantities across Yemen Red Crescent warehouses</p>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <select 
          ...
          class="px-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 text-slate-700 font-semibold"
        >
        ...
        <table class="w-full text-left text-xs">
          <thead>
            <tr class="bg-slate-50/80 border-b border-slate-200/80 text-slate-500 font-semibold uppercase tracking-wider">
              ...
              <th class="py-3.5 px-6 text-right">Actual Qty</th>
              <th class="py-3.5 px-6 text-right">Reserved Qty</th>
              <th class="py-3.5 px-6 text-right">Projected Qty</th>
              <th class="py-3.5 px-6 text-right">Valuation Rate</th>
              <th class="py-3.5 px-6 text-right">Stock Status</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100 text-slate-700 font-medium">
            ...
            >
              ...
              <td class="py-3.5 px-6 text-right font-mono text-slate-500">{{ b.reserved_qty || '0' }}</td>
              <td class="py-3.5 px-6 text-right font-mono font-semibold text-slate-700">{{ b.projected_qty || b.actual_qty }}</td>
              <td class="py-3.5 px-6 text-right font-mono text-slate-600">{{ Number(b.valuation_rate || 0).toFixed(2) }}</td>
